...
```sh
npx tsx --env-file=.env.local scripts/sync-content.ts 2>&1
```
...
6. **New** files are sent through AI metadata generation (`scripts/generate-metadata.ts`) and inserted as `draft` status